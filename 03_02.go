...
var AccessLogsHistory AccessLogResponse
...
func main() {
	...
	http.HandleFunc("/address/access_logs", jsonLogsHistory)
...
func saveAccessLogToDB(postalCode string) {
	...
	//postalCodeに検索した回数をカウント
	incrementRequestCounter(postalCode)
...
func incrementRequestCounter(postalCode string) {

	incrementCount := 0

	for index, history := range AccessLogsHistory.AccessLogs {
		if history.PostalCode == postalCode {
			AccessLogsHistory.AccessLogs[index].Count++
			//fmt.Println("req.PostalCode, req.count: ", req.PostalCode, requestCounter[index].Count)
			incrementCount++
		}
	...
	if incrementCount == 0 {
		newPostalCode := PostalRequestCounter{PostalCode: postalCode, Count: 1}
		AccessLogsHistory.AccessLogs = append(AccessLogsHistory.AccessLogs, newPostalCode)
	}

	// AccessLogHistoryをcount順にソート
	sort.Slice(AccessLogsHistory.AccessLogs, func(i, j int) bool {
		return AccessLogsHistory.AccessLogs[i].Count > AccessLogsHistory.AccessLogs[j].Count
	})

}

func jsonLogsHistory(w http.ResponseWriter, r *http.Request) {
    // 集計結果を JSON 形式でレスポンスとして返す
    jsonAccessHistory, err := json.MarshalIndent(AccessLogsHistory, "", "    ")
    if err != nil {
